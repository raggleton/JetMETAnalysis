...
for item_dict in items:
    for flav in flavs:
        item_dict['flav'] = flav
        cmd = 'jet_draw_energyfractions_x  '\
            '-outputFormat ".pdf" '\
            '-filename "{filedir}/{stem}_{append}_{flav}.root" '\
            '-title "{title}" '\
            '-outputDir "{filedir}/EFPlots_{stem}_{append}_{flav}"'.format(**item_dict)
        # print cmd
