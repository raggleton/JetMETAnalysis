...
# output_dir = "QCD_Pt_Herwig_NoJEC_relPtHatCut5_jtptmin4_withPF_Summer16_07Aug2017_V10"
    ...
    for ind, group in enumerate(grouper(glob(pattern), Nfiles, "")):
        ...
        for algo in all_algos:
            args_dict = {
                ...
                # "input": os.path.join(input_dir, "*.root"), 
                "input": " ".join(group).strip(),
                "output": "%s/jaj_%s_%s_L1FastJet_%d.root" % (this_output_dir, name, algo.split(":")[0], ind),
                "algos": algo,
                "era": jec_era,
                "jecpath": jec_path,
                "levels": jec_levels,
            }
            cmd = "qsub -N {name} -v INPUT='{input}',OUTPUT={output},ALGOS='{algos}',ERA={era},JECPATH={jecpath},LEVELS={levels} do_jet_apply_jec_x_job.sh".format(**args_dict)
            print cmd
