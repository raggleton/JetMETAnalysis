...
append = "_standardMedianErr_allMedian_rspRangeLarge_fitMin%d_fitFunc%s_useFitRange_absEta_hadronParton" % (fit_min, fit_func)
...
for name, input_file in infos:
    ...
    for algo in all_algos:
        for flav in flavours:
            ...
            cmd = 'jet_l2_correction_x -input "{INPUT}" -algs "{ALGOS}" -l2l3 true -histMet median -era "{ERA}" -outputDir "{OUTPUTDIR}" -output "{OUTPUTFILE}" -batch true -flavor "{PDGID}" -fitMin "{FITMIN}" -l2pffit "{FITFUNC}"'.format(**args_dict)
            print cmd
